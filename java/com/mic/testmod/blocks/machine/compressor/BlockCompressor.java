...
public class BlockCompressor extends HardBlock implements ITileEntityProvider {
	...
	public BlockCompressor(String name) {
		super(name, Material.IRON, 1.0f, 0, 0);
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand and, EnumFacing facing, float hitX, float hitY, float hitZ){
		...
		if(!worldIn.isRemote){
			playerIn.openGui(Main.instance, Reference.GUI_COMPRESSOR, worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
	...
	public static void setState(boolean active, World worldIn, BlockPos pos){
		...
		if(active) worldIn.setBlockState(pos, ModBlocks.COMPRESSOR.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(BURNING, true), 3);
		else worldIn.setBlockState(pos, ModBlocks.COMPRESSOR.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(BURNING, false), 3);
