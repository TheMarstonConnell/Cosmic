...
public class TileEntityCosmicForge extends TileEntity implements IInventory, ITickable
{
	...
	{
		return this.hasCustomName() ? this.customName : "container.cosmic_forge";
	}
	...
	public void update() 
	{
		...
		if(!this.world.isRemote) 
		{
			...
			{
				flag1 = true;
				BlockCosmicForge.setState(this.isBurning(), this.world, this.pos);
			}
		} 
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = CosmicForgeRecipes.getInstance().getForgeResult((ItemStack)this.inventory.get(0), (ItemStack)this.inventory.get(1));	
	...
	public void smeltItem() 
	{
		if(this.canSmelt()) 
		{
			...
			ItemStack result = CosmicForgeRecipes.getInstance().getForgeResult(input1, input2);
			ItemStack output = (ItemStack)this.inventory.get(3);
	...
	public static int getItemBurnTime(ItemStack fuel) 
	{
		...
		else 
		{
			...
			if (item instanceof ItemBlock && Block.getBlockFromItem(item) != Blocks.AIR) 
			{
				...
				if (block == Blocks.SEA_LANTERN) return 2300;
				if (block == Blocks.DIAMOND_BLOCK) return (int) (3200 * 9.5);
			}

			if (item == Items.PRISMARINE_CRYSTALS) return 1000;
			if (item == Items.ENDER_PEARL) return 1600;
			if (item == Items.ENDER_EYE) return 2600;
			if (item == Items.DIAMOND) return 3200;
